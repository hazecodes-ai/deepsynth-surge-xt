                                      ...
                                      Surge::GUI::IComponentTagValue *launchFrom)
{
    ...
    // DeepSynth AI Assistant
    auto deepSynthMenu = juce::PopupMenu();

    deepSynthMenu.addItem(Surge::GUI::toOSCase("Show Panel"), [this]() {
        std::cout << "DeepSynth panel toggle clicked!" << std::endl;
        // Toggle the visibility of the DeepSynth panel (no window resizing)
        if (deepSynthPanel)
        {
            bool willBeVisible = !deepSynthPanel->isVisible();
            deepSynthPanel->setVisible(willBeVisible);
            
            // No window resizing - panel appears as overlay
        }
    });

    deepSynthMenu.addSeparator();

    deepSynthMenu.addItem(Surge::GUI::toOSCase("API Config..."), [this]() {
        std::cout << "DeepSynth API Config clicked!" << std::endl;
        // Show API configuration dialog
        if (deepSynthPanel)
        {
            deepSynthPanel->showApiSettings();
        }
    });

    settingsMenu.addSubMenu(Surge::GUI::toOSCase("DeepSynth"), deepSynthMenu);
