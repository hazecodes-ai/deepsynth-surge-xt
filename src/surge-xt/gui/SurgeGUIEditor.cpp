...
#include "panels/DeepSynthPanel.h"
...
void SurgeGUIEditor::openOrRecreateEditor()
{
    ...
    // Create DeepSynth panel in the right sidebar
    if (!deepSynthPanel)
    {
        deepSynthPanel = std::make_unique<Surge::GUI::Panels::DeepSynthPanel>(this, &synth->storage);
        
        // Restore saved state
        Surge::GUI::Panels::DeepSynthPanel::State state;
        state.hasResponse = deepSynthPanelState.hasResponse;
        state.lastResponseText = deepSynthPanelState.lastResponseText;
        state.lastPromptText = deepSynthPanelState.lastPromptText;
        deepSynthPanel->setState(state);
        
        // Position in bottom-right corner
        int deepSynthWidth = 300; // Wider panel for better usability
        int deepSynthHeight = 400; // Fixed height
        int deepSynthX = currentSkin->getWindowSizeX() - deepSynthWidth - 10; // Right edge with margin
        int deepSynthY = currentSkin->getWindowSizeY() - deepSynthHeight - 10; // Bottom edge with margin
        
        deepSynthPanel->setBounds(deepSynthX, deepSynthY, deepSynthWidth, deepSynthHeight);
        deepSynthPanel->setVisible(false); // Start hidden, show via menu
        
        // Add directly to the juce editor window
        if (juceEditor)
        {
            juceEditor->addAndMakeVisible(*deepSynthPanel);
        }
    }
...
void SurgeGUIEditor::close_editor()
{
    ...
    // Clean up DeepSynth panel
    if (deepSynthPanel)
    {
        // Save state before destroying
        deepSynthPanelState.hasResponse = deepSynthPanel->getState().hasResponse;
        deepSynthPanelState.lastResponseText = deepSynthPanel->getState().lastResponseText;
        deepSynthPanelState.lastPromptText = deepSynthPanel->getState().lastPromptText;
        deepSynthPanel.reset();
    }
}
...
void SurgeGUIEditor::setZoomFactor(float zf, bool resizeWindow)
{
    ...
    // Update DeepSynth panel position when zoom changes
    if (deepSynthPanel && deepSynthPanel->isVisible() && currentSkin)
    {
        int deepSynthWidth = 300;
        int deepSynthHeight = 400;
        int deepSynthX = currentSkin->getWindowSizeX() - deepSynthWidth - 10;
        int deepSynthY = currentSkin->getWindowSizeY() - deepSynthHeight - 10;
        deepSynthPanel->setBounds(deepSynthX, deepSynthY, deepSynthWidth, deepSynthHeight);
    }
}
