...
#include "overlays/ClaudePromptDialog.h"
...
std::unique_ptr<Surge::Overlays::OverlayComponent> SurgeGUIEditor::createOverlay(OverlayTags olt)
{
    ...
    case CLAUDE_PROMPT_DIALOG:
    {
        auto cpd = std::make_unique<Surge::Overlays::ClaudePromptDialog>(this, &(this->synth->storage));
        cpd->setEnclosingParentTitle("DeepSynth");
        
        auto posRect = juce::Rectangle<int>(0, 0, 600, 400).withCentre(frame->getBounds().getCentre());
        cpd->setEnclosingParentPosition(posRect);
        cpd->setHasIndependentClose(true);
